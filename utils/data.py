...
class DataLoad():
    ...
    def processing_data(self):
        ...
        label_test[self.target] = label_test[self.target].apply(lambda x: 1 if x > 0 else 0)
        label_train[self.target] = label_train[self.target].apply(lambda x: 1 if x > 0 else 0)
    ...
    def get_dataloader(self, x_train, x_val, y_train, y_val, images_test, label_test):
        ...
        train_loader = data.DataLoader(train_dataset, batch_size=self.batch_size, shuffle=False)
