...
class Train():
    def __init__(self):
        ...
        self.target = config.trainer10['target']
        self.path = config.trainer10['path']
        self.params = config.trainer10
    ...
    def model(self):
        model = CNN()
        model = initialize_parameters(model, 'leaky_relu') # Xavier init
        ...
        self.scheduler = ReduceLROnPlateau(self.optimizer, factor=self.params['sheduler_factor'], patience=self.params['sheduler_patience'], verbose=True)
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            self.scheduler.step(loss_val)     # 更新学习率
    ...
    def evaluate(self):
        ...
        plt.legend()
        plt.savefig('./reports/figures/trainer10_CNN_classification_loss.png')
        plt.show()
        ...
        plt.savefig('./reports/figures/trainer10_CNN_classification_train_val_loss.png')
        plt.show()
        ...
        cm = confusion_matrix(self.label_test[self.params['target']], self.label_test['pred'])
        print("Test Accuracy", (cm[0][0]+cm[1][1])/np.sum(cm))
        print('Test Loss', np.mean(global_loss_test))
        print("Test Pearsonr:", scipy.stats.pearsonr(self.label_test[self.params['target']], self.label_test['pred'])[0])
        print("Test Spearmanr:", scipy.stats.spearmanr(self.label_test[self.params['target']], self.label_test['pred'])[0])
        ...
        cm = confusion_matrix(self.y_val[self.params['target']], self.y_val['pred'])
