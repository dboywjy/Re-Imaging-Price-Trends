import sys
sys.path.append("..")
from models.model_reg2 import *
from utils.utils import *
from utils.data import DataLoad2
import config
...
class Train():
    def __init__(self):
        ...
        self.target = config.trainer_reg2['target']
        self.path = config.trainer_reg2['path']
        self.params = config.trainer_reg2

    def dataStep(self):
        data = DataLoad2(self.IMAGE_WIDTH, self.IMAGE_HEIGHT, self.train_val_years, self.test_years, self.target, self.path, self.params['batch_size'])
        [self.x_train, self.x_val, self.y_train, self.y_val, self.images_test, self.label_test] = data.processing_data()
        [self.train_loader, self.val_loader, self.test_loader] = data.get_dataloader(self.x_train, self.x_val, self.y_train, self.y_val, self.images_test, self.label_test)
    
    def model(self):
        model = CNN()
        self.device = torch.device("cuda:2" if torch.cuda.is_available() else "cpu")
        self.device_ids=range(torch.cuda.device_count())
        model.cuda(device=self.device_ids[2])
        self.model = model 
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            for i,(x,y) in enumerate(self.train_loader):
                batch_x = Variable(x.cuda(device=self.device_ids[2]))
                batch_y = Variable(y.cuda(device=self.device_ids[2]))
                out = self.model(batch_x.float())
                loss = self.loss_func(out,batch_y.view(-1,1).float())
            ...
            for x,y in self.val_loader:
                ...
                loss = self.loss_func(prediction, batch_y.view(-1,1).float())
    ...
    def evaluate(self):
        ...
        plt.legend()
        plt.savefig('./reports/figures/trainer_reg2_CNN_classification_loss.png')
        plt.show()
        ...
        plt.savefig('./reports/figures/trainer_reg2_CNN_classification_train_val_loss.png')
        plt.show()
        ...
        with torch.no_grad():
            for images,labels in self.test_loader:
                test_x = Variable(images.cuda(device=device_ids[2]))
                test_y = Variable(labels.cuda(device=device_ids[2]))
                prediction_test = self.model(test_x)
                prediction.extend(torch.max(prediction_test,1)[1].cpu().numpy().tolist())
                loss_test = self.loss_func(prediction_test, test_y.view(-1,1).float())
                global_loss_test.append(loss_test.cpu().detach().numpy())
        self.label_test['pred'] = prediction
#         cm = confusion_matrix(label_test[self.params['target']], label_test['pred'])
        print("Test R2", r2_score(self.label_test[self.params['target']], self.label_test['pred']))
        print('Test Loss', np.mean(global_loss_test))
        print("Test mean_squared_error", mean_squared_error(self.label_test[self.params['target']], self.label_test['pred']))
        #########################################################################
        prediction = []
        global_loss_val = []
        with torch.no_grad():
            for images,labels in self.val_loader:
                val_x = Variable(images.cuda(device=device_ids[2]))
                val_y = Variable(labels.cuda(device=device_ids[2]))
                prediction_val = self.model(val_x)
                prediction.extend(torch.max(prediction_val,1)[1].cpu().numpy().tolist())
                loss_val = self.loss_func(prediction_val, val_y.view(-1,1).float())
                global_loss_val.append(loss_val.cpu().detach().numpy())
        self.y_val['pred'] = prediction
#         cm = confusion_matrix(y_val[self.params['target']], y_val['pred'])
        print("Val R2", r2_score(self.y_val[self.params['target']], self.y_val['pred']))
        print('Val Loss', np.mean(global_loss_val))
        print("Val mean_squared_error", mean_squared_error(self.y_val[self.params['target']], self.y_val['pred']))
        #########################################################################
