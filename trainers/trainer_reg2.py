...
class Train():
    ...
    def evaluate(self):
        ...
        data = DataLoad2(config.IMAGE_WIDTH, config.IMAGE_HEIGHT, config.train_val_years, config.test_years, self.params['target'], self.params['path'], self.params['batch_size'])
...
#         cm = confusion_matrix(label_test[self.params['target']], label_test['pred'])
        print("Test R2", r2_score(label_test[self.params['target']], label_test['pred']))
        print('Test Loss', np.mean(global_loss_test))
        print("Test mean_squared_error", mean_squared_error(label_test[self.params['target']], label_test['pred']))
        #########################################################################
        prediction = []
        global_loss_val = []
        with torch.no_grad():
...
#         cm = confusion_matrix(y_val[self.params['target']], y_val['pred'])
        print("Val R2", r2_score(y_val[self.params['target']], y_val['pred']))
        print('Val Loss', np.mean(global_loss_val))
        print("Val mean_squared_error", mean_squared_error(y_val[self.params['target']], y_val['pred']))
        #########################################################################
