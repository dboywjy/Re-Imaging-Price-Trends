...
class Train():
    def __init__(self):
        ...
        self.target = config.trainer8['target']
        self.path = config.trainer8['path']
        self.params = config.trainer8
    ...
    def model(self):
        ...
        model.cuda(device=self.device_ids[1])
        self.model = model 
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            for i,(x,y) in enumerate(self.train_loader):
                batch_x = Variable(x.cuda(device=self.device_ids[1]))
                batch_y = Variable(y.cuda(device=self.device_ids[1]))
    ...
    def evaluate(self):
        ...
        plt.legend()
        plt.savefig('./reports/figures/trainer8_CNN_classification_loss.png')
        plt.show()
        ...
        plt.savefig('./reports/figures/trainer8_CNN_classification_train_val_loss.png')
        plt.show()
        ...
        with torch.no_grad():
            for images,labels in self.test_loader:
                test_x = Variable(images.cuda(device=device_ids[1]))
                test_y = Variable(labels.cuda(device=device_ids[1]))
        ...
        with torch.no_grad():
            for images,labels in self.val_loader:
                val_x = Variable(images.cuda(device=device_ids[1]))
                val_y = Variable(labels.cuda(device=device_ids[1]))
